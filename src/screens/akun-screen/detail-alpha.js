...
function App({navigation, route}){
    ...
    const [alphalink, setLinkalpha] = useState("");
    ...
    useEffect(() => {
        axios.get(`https://kispros.com/api/get_api_detail_alpha/${route.params.no_hp}`).then((res) => {
            
                setData(res.data);
                setIsload(true);
        axios.get(`https://kispros.com/api/get_donasi/${route.params.no_hp}`).then((response) => {
    
            setLinkalpha(response.data.linkalpha);
        });
        ...
        return isload ? (
            data.length > 0 ? (
                                        ...
                                        <Card style={{marginBottom:5}}>
                                            <View style={{flexDirection:'row'}}>
                                                <View style={{flexDirection:'column', width:'50%', textAlign:'left', justifyContent:'center'}} >
                                                    <Image style={{height:55, width:70}} source={{uri:`${GAMBAR}/alpha.jpg`}}></Image>
                                                </View>
                                                    ...
                                                    <Text style={{textAlign:'right', fontSize:16}}>{item.jml ? 'Rp. '+rupiah(item.jml) : 'Belum ada'}</Text>
                                                        ...
                                                        <View style={{flexDirection:'row', width:'100%'}}>
                                                            ...
                                                            <View style={{flexDirection:'column', width:'50%'}}>
                                                                ...
                                                                {fangsen(item.ratealpha).map((dt, index) => {
                                                                    return dt; 
                                                                })}
                                                                </View>
                ...
                <View style={{backgroundColor:'white', alignItems:'center', justifyContent:'center', flex:1}}>
                    <Text status='success' category='h5' style={{padding:10}}>Belum ada donasi nih</Text>
                    <View style={{flexDirection:'row'}}>
                        <Icon
                            ...
                            fill='grey'
                        />
                        <Text status='info' onPress={() => Linking.openURL(alphalink)}>Yuk hubungi admin kita disini </Text>
                        <Icon
